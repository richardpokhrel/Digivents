...
const WhyChooseUsSlide = ({
  ...
  description = 'At DigiVents, we stand out by delivering tailored solutions, unmatched expertise, and a passion for innovation that drives results.',
  ctaText = 'Work With Us',
  ctaLink = '/contact',
  companyName = 'DigiVents',
  features = [
    { icon: 'fas fa-cog', title: 'Tailored Solutions', description: 'Customized strategies to meet your unique needs.' },
    { icon: 'fas fa-star', title: 'Proven Expertise', description: 'Years of experience delivering top-tier results.' },
    { icon: 'fas fa-heart', title: 'Client-Centric', description: 'Your success is our top priority.' },
  ],
}) => {
  return (
    <section
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <header className="text-center space-y-6 sm:space-y-8 max-w-4xl mx-auto animate-slideIn">
          <div className="inline-flex items-center bg-pink-100 text-pink-600 px-3 py-1.5 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-semibold shadow-sm" role="note">
            ...
            <span>Why Choose {companyName}?</span>
          </div>
          <h1
          ...
          <h2 className="text-lg sm:text-xl md:text-2xl lg:text-3xl text-gray-600 font-medium max-w-3xl mx-auto">
            {subtitle}
          </h2>
        ...
        <div className="relative max-w-6xl mx-auto mt-12 sm:mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 px-4 sm:px-0">
          {features.map((feature, index) => (
            ...
            >
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-[#510a3d] rounded-full flex items-center justify-center text-white mx-auto mb-4">
                <i className={`${feature.icon} text-lg sm:text-xl`} aria-hidden="true"></i>
              </div>
              <h3 className="text-lg sm:text-xl font-bold text-[#510a3d] mb-3">{feature.title}</h3>
          ...
          <div className="absolute inset-0 bg-gradient-to-br from-[#510a3d] to-pink-400 rounded-3xl opacity-10 transform rotate-2 scale-105 -z-10" aria-hidden="true"></div>
        </div>

        {/* CTA */}
        <div className="text-center mt-10 sm:mt-12 relative z-20">
          <Link
            to={ctaLink}
            className="group inline-flex items-center justify-center bg-[#510a3d] text-white px-5 py-2.5 sm:px-7 sm:py-3.5 rounded-full text-sm sm:text-base font-semibold hover:bg-[#6b1a4f] hover:shadow-lg hover:scale-105 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-[#510a3d] focus:ring-offset-2"
            aria-label={`Start working with ${companyName} to achieve ${ctaText.toLowerCase()}`}
          >
            <span>{ctaText}</span>
            <i className="fas fa-arrow-right ml-2 transform group-hover:translate-x-1 transition-transform" aria-hidden="true"></i>
          </Link>
        </div>
      </div>
      ...
      <img
        ...
        alt="Abstract decorative shape for Why Choose Us section"
      ...
      <img
        ...
        sizes="(max-width: 768px) 100px, 150px"
        alt="Abstract decorative shape enhancing Why Choose Us section"
